#!/usr/bin/env python3
"""Compute the next version tag for a branch, per SpooNMAP's release policy.
...
def parse_final(tag: str) -> Optional[Version]:
...
def target_version(base: Version, messages: List[str]) -> Optional[Version]:
...
def compute(channel: str, tags: List[str], messages: List[str]) -> Optional[str]:
