...
- **Resume target coverage**: an mtime baseline records *when* a file changed, never *what target set the cached scan covered*, so it cannot catch a target set that changes without its baseline file being rewritten. Each phase whose `-iL` target is discovery-derived therefore also records that target beside its output — `<output>.target`, written by `_stamp_target_coverage()` — and `_resume_cache_usable(output_file, baseline_mtime, description, target_file, ...)` rejects the cache unless the recorded set covers what this run would scan. `target_file` is a **required** positional, not an opt-in keyword, so every new gate has to state its choice. Six phases are covered: the Full sweep, the port batches, `_nmap_port_discovery()`, `_nmap_udp_discovery()`, and `nmap_scan()`'s banner and NSE passes. Two under-scans this closes, both of which reported a narrow scan as a complete one: (1) toggling `host_discovery` off on a `--resume` run widens the target from the discovery file to the whole range while nothing rewrites `resolved_targets.txt`, so every cached phase still satisfied its gate; (2) the port batches, with **no config change at all** — `live_hosts_combined.txt` is rebuilt every run from a probe that is deliberately not resume-gated, so a host run 2's probe found but run 1's missed reached `live_hosts/` and `all_live_hosts.txt` yet was never scanned for the already-cached batches' ports.
- **Why target coverage is a subset test, not equality**: the gate accepts when `current ⊆ stamped` — everything this run would scan was already covered — which is why the sidecar stores the normalised target list rather than a digest of it. Equality looked simpler and was wrong: the batch phase's target moves in *both* directions between runs, because the iterative single-port probe (`batch_size == 1`) stops at the first port that finds hosts, so which ports land in `probe_ports_used` — and therefore which cached hosts get folded into `live_hosts_combined.txt` — varies with probe luck. Under equality an ordinary difference in packet loss re-scanned every completed batch against a *narrower* target than the cache it discarded, i.e. `--resume` stopped resuming. The comparison is over normalised **lines** (sorted, blank- and comment-stripped), not addresses, so `10.0.0.0/24` and an explicit list of its hosts do not match even though they scan the same range; that only ever errs toward a redundant re-scan. Rejections name the uncovered targets so the reason is visible rather than inferred.
- **Where target coverage is deliberately *not* checked** — three gates pass `target_file=None`, and only the first is safe. `_host_discovery()`: its target *is* the mtime baseline (`resolved_targets.txt`, written through `_write_if_changed()`), so an mtime bump and a content change are already equivalent. Separately, **no** gate covers the exclusions file: `--excludefile` is passed straight to masscan/nmap and never enters the fingerprinted `-iL` file, so narrowing `exclusions.txt` between runs widens the real scan with nothing to notice it — and `_host_discovery()`'s effective target is the exclusion-subtracted `filtered_target`, not the `resolved_targets.txt` its gate is keyed to. Both are the next instances of this same pattern; do not read the four covered phases as "resume is now safe against any input change".
- **The banner and NSE passes** are coverage-checked against `live_hosts/portNN.txt`, with the record written from `nmap_worker()` on each pass's own success path. That list is written as a **union** by the probe, the batch phase and the Full sweep, so it grows across runs and a cached `portN.xml` that merely parses is no evidence it covered the hosts now in it — the probe is deliberately not resume-gated, so an ordinary re-probe adds a host that a bare existence check would never banner-scan. This was the gap that limited the masscan-side fix: masscan decides what reaches `live_hosts/` at all, but a banner scan is how the tool is normally run, so the newly-masscanned host still got no service data. The two passes record independently — one succeeding says nothing about the other — and the record keys on the IP list even when `nmap_worker()` hands nmap the `portNN_hostnames.txt` variant, because that file is a pure derivation of the IP list, regenerated every run, and does not exist yet when the gate runs.
- **Stamping rules**: written on **success paths only**, exactly as `_EMPTY_RESULT_XML` is — a stamp on a killed scan would assert coverage that never happened. A `KeyboardInterrupt`, a missing binary, and a non-zero exit all leave none; `_nmap_udp_discovery()` needs an explicit `proc.returncode == 0` guard for that last one, because unlike `_nmap_port_discovery()` it does not treat a non-zero exit as fatal and a failing nmap can still leave parseable partial XML. Neither an unreadable target file nor a failed sidecar write raises: the scan already succeeded, so unwinding would discard real results, and the cost either way is one redundant re-scan. Both failure paths **delete** any existing record via `_discard_target_stamp()` rather than leaving it: outputs are rewritten in place across runs, and a failed nmap pass is renamed aside by `_quarantine_failed_output()`, so a record that survived a failed write would be read as describing output it never covered — a later wider scan validated by an earlier narrower record. A cache with **no** sidecar is rejected too, so output from before this change re-scans once on the first resume after upgrading — the deliberate direction, since a redundant re-scan is visible and an under-scan is not. The `.target` suffix is invisible to every result consumer: `masscan_results/` is aggregated by listing the directory, and `_parse_result_xml()` drops anything not ending in `.xml`, the same guard that already hides `portN.xml.failed`. `_delete_previous_results()` removes the sidecars with the directory, so `--cleanup` and `[d]elete` need no special handling.
